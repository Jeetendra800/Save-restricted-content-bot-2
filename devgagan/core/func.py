# ---------------------------------------------------
# File Name: func.py (FINAL FIXED VERSION)
# ---------------------------------------------------
...
# --- Missing video_metadata Function Added Back ---
def video_metadata(file):
    default_values = {'width': 1, 'height': 1, 'duration': 1}
    try:
        vcap = cv2.VideoCapture(file)
        if not vcap.isOpened(): return default_values  
        ...
        vcap.release()
        return {'width': width, 'height': height, 'duration': duration if duration > 0 else 1}
    except Exception:
...
async def prog_bar(current, total, ud_type, message, start):
    # This is often used as an alias for progress_bar
    return await progress_bar(current, total, ud_type, message, start)
...
async def screenshot(video, duration, sender):
    if os.path.exists(f'{sender}.jpg'):
        return f'{sender}.jpg'
    time_stamp = time.strftime('%H:%M:%S',time.gmtime(int(duration)/2))
...
async def progress_callback(current, total, progress_message):
    ...
    if current_time - last_update_time >= 10:
        ...
        await progress_message.edit(
            ...
            f"│ **__Uploaded:__** {humanbytes(current)} / {humanbytes(total)}\n"
            f"╰──────────────────╯\n\n**__Powered by RAJPUTSERVER__**"
        )
